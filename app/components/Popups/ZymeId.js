...
function ZymeId(props) {
  ...
  const updateAccount = (event) => {
    ...
    if (account.zymeId.value != "-") {
      setIsInvalid(false);
      const obj = {
        ATTRIBUTE: "VALID_ZYME_ID",
        GOLDEN_ACCOUNT_ID: props.goldenAccountId,
        CURRENTVALUE: props.currentValue,
        NEWVALUE: account.zymeId.value,
  ...
  useEffect(() => {
    getChangeHistory(props.goldenAccountId, "VALID_ZYME_ID").then((res) => {
      if (res && res.data && res.data.data) {
        setHistory(res.data.data);
      }
    });
    getAttributes(props.goldenAccountId, "VALID_ZYME_ID").then((res) => {
      let opt = [
        {
          label: "-",
          value: "-",
      ...
      if (res && res.data && res.data.data) {
        _.each(res.data.data, (arr) => {
          if (arr.VALID_ZYME_ID != props.currentValue) {
            opt.push({
              label: arr.VALID_ZYME_ID,
              value: arr.VALID_ZYME_ID,
            });
          }
        });
      }
  ...
  return (
    <Model
      ...
      title={"Edit Valid Zyme Id"}
    >
      <form>
        <FormField>
          <label class="MandatoryLabel">Valid Zyme ID</label>
          <SelectBox
            data={options}
            onSelect={_onSelect}
            defaultOption={account.zymeId}
          />
        </FormField>
        <FormField>
